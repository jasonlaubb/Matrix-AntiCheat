...
import { antiSpeedMineEnabled,toolsNames,stoneTools,woodTools } from "../../config"
...
if (stoneTools.includes(item)){
  ...
  breakSpeed = 0
    ...
    system.run(()=>{ 
    	...
    	system.run(()=>{ 
    	if(player.hasTag("MatrixOP") || getNuke>=2 || breakTimer > breakSpeed || breakSpeed == 0 || speedMineToggle != true) return 
        })
    	event.cancel = true 
    system.run(()=>{ 
 ...
 if (breakFlags > 4){system.run(()=>{ 
    	if(getNuke>=2 || breakTimer > breakSpeed || breakSpeed == 0 || player.hasTag("MatrixOP") || speedMineToggle != true || fastBrokenBlocks.includes(block.type.id)) return 
    	player.runCommand(`scoreboard players set @s mineFlags 0`) 
    	player.runCommand(`tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §gSpeed mine §8(§gA§8) §chas been detected from §b${player.name}\n§cBlocks §8= §8(§g${breakFlags} blocks§8/§gsecond§8)"}]}`) 
   }) }	
