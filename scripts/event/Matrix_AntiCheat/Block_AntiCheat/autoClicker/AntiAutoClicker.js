...
import { antiAutoClickerEnabled,maximumCpsPlace } from "../../config"
...
if (antiAutoClickerEnabled == true){
 ...
 let  blockId = block.type.id.replaceAll("minecraft:",""); 
    ...
    if (getCps >= maximumCpsPlace){
        ...
        player.runCommand(`tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §can unNatural §gClicking §8(§gB§8) §chas been detected from §b${player.name}\n§cCps §8= §8(§g${getCps}§8/§g${maximumCpsPlace}§8)\n§cBlock §8 = §8(§g${blockName}§8)"}]}`) 
