...
import { antiNukerEnabled,nukerTimer } from "../../config"
...
if (antiNukerEnabled == true){
  ...
  let getNukeTime = world.scoreboard.getObjective("nukerTimer").getScore(player.scoreboardIdentity) 
    ...
    if (getNukeTime >= 1){
    	...
    	event.cancel = true
    	} if(checkEfficiency != null && getNukeTime >= 1 || player.getEffect("haste") && getNukeTime >= 1 || fastBrokenBlocks.includes(block.type.id) && getNukeTime >= 1 ){
    	system.run(()=>{ 
