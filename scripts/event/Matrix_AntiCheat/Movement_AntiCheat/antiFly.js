...
let flyToggle;
export async function antiFly (player) {
  try {
    try {
    ...
    if (antiFlyEnabled == true) {
      ...
      if (flyTimer > 14) {
        ...
        player.runCommand(
          `tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §can unNatural Movement §gFly §8(§gb§8) §chas been detected from §b${player.name} §8(§g${distance.toFixed(2)}§8/§g${maximumDisMovement}§8) §gblocks"}]}`
          )
      ...
      if (theEnd > 0) {
        ...
        if (player.hasTag("sleeping") || velocityY < 0 || player.getEffect("jump_boost") || player.getEffect(
            "levitation") || player.hasTag("MatrixOP")) {
        ...
        if (flyTp == 1 && tryFly > 1) {
          ...
          player.runCommand(
            `tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §can unNatural Movement §gFly §8(§gA§8) §chas been detected from §b${player.name} §8(§g${distance.toFixed(2)}§8/§g${maximumDisMovement}§8) §gblocks"}]}`
            )
        ...
        if (flyFlags >= 4 && distance > maximumDisMovement) {
          ...
          player.addTag(`ban`)
          player.addTag(
            `Reason:§cFly §8(§gA§8) §cDistance §8= (§g${distance.toFixed(2)}§8/§g${maximumDisMovement}§8) §gBlocks`)
