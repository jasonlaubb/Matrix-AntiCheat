...
import { antiFlyEnabled }  from "../config"
...
export async function antiFly(player){
 ...
 distance = Math.sqrt((firstPosY-player.location.y.toFixed(2))*(firstPosY-player.location.y.toFixed(2))) 
    ...
    player.runCommand(`scoreboard players add @s flyTimer 2`) 
        ...
        player.runCommand(`tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §can unNatural Movement §gFly §8(§gb§8) §chas been detected from §b${player.name} §8(§g${distance.toFixed(2)}§8/§g${maximumDisMovement}§8) §gblocks"}]}`) 
