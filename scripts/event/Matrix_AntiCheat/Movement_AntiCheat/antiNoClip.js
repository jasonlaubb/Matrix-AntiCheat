...
import { antiPhaseEnabled }  from "../config"
...
export async function antiPhase(player){
    ...
    let head = player.dimension.getBlock({ x: Math.floor(player.location.x) , y: Math.floor(player.location.y+1), z: Math.floor(player.location.z) })
    let body = player.dimension.getBlock({ x: Math.floor(player.location.x) , y: Math.floor(player.location.y), z: Math.floor(player.location.z) })
    let bypassFP = player.dimension.getBlock({ x: firstPosX , y: firstPosY, z: firstPosZ })
    let bypassFP2 = player.dimension.getBlock({ x: firstPosX , y: firstPosY+1, z: firstPosZ })
...
if(head.isSolid == false && body.isSolid == false){
    ...
    player.runCommand(`scoreboard players set @s phaseZ ${playerZ}`) 
    	...
    	if(bypassFP.isSolid == true || bypassFP2.isSolid == true || body.typeId.includes("soul_sand") && head.isSolid == false || player.hasTag("riding")) return 
        player.runCommand(`scoreboard players set @s skip_check 3`) 
    	player.runCommand(`tp ${firstPosX} ${firstPosY} ${firstPosZ}`) 
    player.runCommand(`tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §can unNatural Movement §gPhase §8(§gA§8) §chas been detected from §b${player.name}"}]}`) 
    	} 
   } 
 } 