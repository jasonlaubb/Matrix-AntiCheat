...
import { antiSpeedEnabled,detect }  from "../config"
...
export async function antiSpeed(player){
    ...
    distance = Math.sqrt((firstPosX-playerX/100)*(firstPosX-playerX/100)+(firstPosZ-playerZ/100)*(firstPosZ-playerZ/100)).toFixed(2)
    ...
    if (normalMove>5){
    	...
    	} if ( player.isGliding == true && velocityY<0 ||  player.hasTag("sleeping") || player.hasTag("riding") || player.getEffect("speed") || player.hasTag("MatrixOP") ){
    	distance = 0
    } 
...
if(distance>maximumDisMovement){
    	...
    	if ( player.isGliding == true && velocityY<0 ||  player.hasTag("sleeping") || player.hasTag("riding") || player.getEffect("speed") || player.hasTag("skip_check") || player.hasTag("skip_checkS") || player.hasTag("MatrixOP") ) return
        player.runCommand(`scoreboard players set @s normalS 0`) 
        } 
        if(theEnd>0){
if(distance>maximumDisMovement){
        ...
        if ( player.isGliding == true && velocityY<0 ||  player.hasTag("sleeping") || player.hasTag("riding") || player.getEffect("speed") || player.hasTag("skip_check") || player.hasTag("skip_checkS") || player.hasTag("MatrixOP") || skipCheck>0 ) return
    	player.runCommand(`tp @s ${firstPosX} ${playerY/100} ${firstPosZ}`) 
        player.runCommand(`tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §can unNatural Movement §gSpeed §8(§gA§8) §chas been detected from §b${player.name} §8(§g${distance}§8/§g${maximumDisMovement}§8) §gblocks"}]}`) 
        ...
        player.runCommand(`scoreboard players set @s restrySpeed 0`) 
      	...
      	if(distance>maximumDisMovement){
        player.runCommand(`scoreboard players set @s normalS 0`) 
      detect(player,"tp","hi",firstPosX+" "+playerY/100+" "+firstPosZ,true,"§e[§cMatrix§e] §can unNatural Movement §gSpeed §8(§gA§8) §chas been detected from §b"+player.name+" §8(§g"+distance+"§8/§g"+maximumDisMovement+"§8) §gblocks")
        player.runCommand(`scoreboard players set @s trySpeed 0`) 
        player.runCommand(`scoreboard players set @s restrySpeed 0`) 
    	} 
   }
} 
      if ( player.isGliding == true && velocityY<0 ||  player.hasTag("sleeping") || player.hasTag("riding") || player.getEffect("speed") ){
      	player.runCommand(`scoreboard players set @s speedZ ${playerZ}`) 
    player.runCommand(`scoreboard players set @s speedX ${playerX}`) 
      	} 
      if (distance>maximumDisMovement ){
