...
import { antiAutoClickerEnabled,maximumCps } from "../../config"
import { system, ItemStack, world, ItemEnchantsComponent,EffectTypes, Vector, Container,Player } from "@minecraft/server"
...
if (attacker.typeId == undefined) return 
 ...
 let tryAutoClicker = world.scoreboard.getObjective("tryAutoClicker").getScore(attacker.scoreboardIdentity) 
   ...
   if (tryAutoClicker >= 5){
   	...
   	attacker.runCommand(`tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §can unNatural §gClicking §8(§gA§8) §chas been detected from §b${attacker.name}\n§cCps §8= §8(§g${illegalCps}§8/§g${maximumCps}§8)\n§cTarget§8 = §8(§g${targetName.replaceAll("minecraft:","").replaceAll("_"," ")}§8)"}]}`) 
...
for (let player of world.getPlayers()) {
  ...
  if (nukerFlag == 1){
  	...
  	player.runCommand(`tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §gNuker §8(§gA§8) §chas been detected from §b${player.name}\n§cBlocks §8= §8(§g${nukerLength+1}§8/§gBlocks§8)"}]}`) 
