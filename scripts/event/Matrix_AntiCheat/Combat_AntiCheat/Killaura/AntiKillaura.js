...
import { antiKillauraEnabled } from "../../config"
import { disXZ }  from "../Reach/AntiReach"
...
if(attacker == undefined) return 
 ...
 if( attacker.hasTag("is_using_item") || is_using_block>0){
	if (attacker.hasTag("MatrixOP")) return 
        ...
        attacker.runCommand(`tellraw @a[tag=notify]{"rawtext":[{"text":"§g[§cMatrix§g] §gkillaura §8(§gA§8) §chas been detected from §b${attacker.name}\n§cTarget§8 = §8(§g${targetName}§8)"}]}`) 
