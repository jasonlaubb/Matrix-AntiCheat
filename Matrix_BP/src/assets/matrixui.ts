...
export default async function (player: Player) {
    ...
    for (const requiredOption of commandSelected.requiredOption) {
        ...
        currentCommand.push(
            isChoice
                ? requiredOption.typeInfo!.arrayRange![result3.formValues![0] as number]
                : isBoolean
                  ? (result3.formValues![0] as number).toString()
                  : notPlayerTarget
                    ? (result3.formValues![0] as string)
                    : playerNameArray[result3.formValues![0] as number]
        );
    }
    for (const optionalOption of commandSelected.optionalOption) {
        ...
        currentCommand.push(
            isChoice
                ? optionalOption.typeInfo!.arrayRange![result.formValues![0] as number]
                : isBoolean
                  ? (result.formValues![0] as number).toString()
                  : notPlayerTarget
                    ? (result.formValues![0] as string)
                    : playerNameArray[result.formValues![0] as number]
        );
    }
