...
new Command()
	...
	.onExecute(async (player, target) => {
		...
		warn(targetPlayer);
		world.sendMessage(
			fastText().addText("§bMatrix§a+ §7> §g").addTranRawText("command.moderation.success", fastText().addText(targetPlayer.name).addText(player.name).addTran("command.moderation.warn")
			.endline().addTran("command.warn.amount", getWarns(targetPlayer)?.toString() ?? "0").build())
			.build()
		)
	})
	.register();
new Command()
	...
	.onExecute(async (player, target) => {
		...
		world.sendMessage(
			fastText().addText("§bMatrix§a+ §7> §g").addTranRawText("command.moderation.success", fastText().addText(targetPlayer.name).addText(player.name).addTran("command.moderation.clearwarn").build())
			.build()
		)
	})
	.register();