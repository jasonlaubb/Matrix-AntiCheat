...
new Command()
	...
	.addOption(rawtextTranslate("command.moderation.level"), rawtextTranslate("command.moderation.level.description"), "integer", {
		lowerLimit: 0,
		upperLimit: 4,
	}, false)
	.onExecute(async (player, player2, levelOption) => {
		...
		if (level === targetLevel || level >= targetLevel) {
			...
			world.sendMessage(fastText().addText("§bMatrix§a+ §7> §g").addTranRawText("command.setadmin.success", fastText().addText(target.name).addText(player.name).addTran("command.setadmin.level.display", levelOption as string).build()).build());
		}
	})