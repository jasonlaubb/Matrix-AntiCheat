...
new Command()
	...
	.onExecute(async (player, key, value) => {
		...
		if (!isValid) {
			player.sendMessage(fastText().addText("§bMatrix§a+ §7> §c").addTran("command.set.invalid", key as string).build());
			return;
		}
		...
		player.sendMessage(fastText().addText("§bMatrix§a+ §7> §g").addTran("command.set.success", key as string, value as string).build());
	})
	.register();