...
function onPlayerSendMessage (event: ChatSendBeforeEvent) {
	...
	const chatRankMessage = Module.config.chatRank.pattern
		.replace("%rank%", ranks.join(", "))
		.replace("%player%", player.name)
		.replace("%message%", messageNormalized);
	system.run(() => {
...
function chatSpamming (player: Player, message: string) {
	...
	if (sendInterval < MIN_SEND_INTERVAL) {
		system.run(() => {
			player.sendMessage(fastText().addText("§bMatrix§a+ §7> §c").addTran("module.chatrank.slow", (sendInterval * 0.001).toFixed(3)).build());
		})
		return true;
	}
