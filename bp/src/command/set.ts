...
export const resetConfig = {
  name: "resetconfig",
  description:
    "Reset all the changed properties (config) saved in dynamic properties",
  requireOp: true,
  optionalParameters: [
    {
      name: "confirmation",
  ...
  execute: (player, [confirmation]) => {
    if (confirmation !== player.name)
      return {
        status: 1,
        message:
          "§7[§aMatrix§7] §fType your player name to continue, add quote if your name includes space.",
      };
    const matches = world
      .getDynamicPropertyIds()
      .filter((id) => id.startsWith("database:"));
    if (matches.length === 0)
      return {
        status: 1,
...
export const getProperty = {
  name: "getproperty",
  description:
    "View 1 of the property (config) saved in data and dynamic properties",
  requireOp: true,
  parameters: [
    {
      name: "property",
