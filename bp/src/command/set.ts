...
export const setBoolean = {
    ...
    execute: (_player, [id, value]) => {
        ...
        return { status: 0, message: `§7[§aMatrix§7] §fSuccessfully changed property §e${id}§f to §e${value ? "true" : "false"}` };
    }
} as Command;
export const setString = {
    ...
    execute: (_player, [id, value]) => {
        system.run(() => world.setDynamicProperty("database:" + id, value));
        return { status: 0, message: `§7[§aMatrix§7] §fSuccessfully changed property §e${id}§f to §e${value}` };
    }
} as Command;
...
export const resetConfig = {
    ...
    execute: (player, [confirmation]) => {
        if (confirmation !== player.name) return { status: 1, message: "§7[§aMatrix§7] §fType your player name to continue, add quote if your name includes space." };
        const matches = world.getDynamicPropertyIds().filter((id) => id.startsWith("database:"));
        if (matches.length === 0) return { status: 1, message: "§7[§aMatrix§7] §fYou have never changed any property..." }
        system.run(() => matches.forEach((id) => world.setDynamicProperty(id)));
        return { status: 0, message: "§7[§aMatrix§7] §fSucessfully reset the config." };
    }
} as Command;
export const clearProperty = {
    ...
    execute: (_player, [id]) => {
        if (!world.getDynamicProperty("database:" + id)) return { status: 1, message: "§7[§aMatrix§7] §fTarget property has not been changed." }
        system.run(() => world.setDynamicProperty("database:" + id));
        return { status: 0, message: "§7[§aMatrix§7] §fSucessfully reset the target property." }
    }
} as Command;
