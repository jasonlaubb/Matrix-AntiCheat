...
export function writeCommandLog(player: string, command: string) {
    world.setDynamicProperty("log:command:" + Date.now(), `${player};${command}`);
}
export function writeCommandBlockLog(player: string, place: boolean, commandblock: Vector3, dimension: string) {
    world.setDynamicProperty("log:commandblock:" + Date.now(), `${place};${Object.values(commandblock).join(" ")};${dimension};${player}`);
}