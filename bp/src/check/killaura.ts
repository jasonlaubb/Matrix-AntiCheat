...
function recordPosition(entity: Entity) {
  ...
  const id = system.runInterval(() => {
    if (
      !entity?.isValid ||
      !entity.antiReachRecordTime ||
      entity.antiReachRecordTime < Date.now()
    ) {
      system.clearRun(id);
      return;
    }
...
function entityHurt({
  ...
  if (!attacker.killauraHitList.map(({ id }) => id).includes(hurtEntity.id))
    attacker.killauraHitList.push({ id: hurtEntity.id, time: now });
  attacker.killauraHitList = attacker.killauraHitList.filter(
    ({ time }) => now - time <= 100,
  );
  if (attacker.killauraHitList.length >= 2) {
    attacker.killauraFlag++;
    attacker.killauraLastFlag = now;
    if (attacker.killauraFlag >= 2)
      attacker.flag("Killaura", "A", "Combat (Multi-aura)");
    attacker.killauraHitList = [];
  ...
  if (
    attacker?.antiReachRecords &&
    attacker.antiReachRecords.length >= 10 &&
    hurtEntity?.antiReachRecords &&
    hurtEntity.antiReachRecords.length >= 10
  ) {
  ...
  if (attackDistance > 3 && fastAbs(pitch) > 75) {
    ...
    attacker.killauraLastFlag = now;
    if (attacker.killauraFlag >= 2)
      attacker.flag("Killaura", "C", "Combat", { attackDistance, pitch });
    addHP(hurtEntity, damage);
  }
  if (attackDistance > 3.5) {
    const angle = calculateRelativeViewAngle(
    ...
    if (angle > (attacker.inputInfo.lastInputModeUsed === "Touch" ? 135 : 85)) {
      ...
      attacker.killauraLastFlag = now;
      if (attacker.killauraFlag >= 3)
        attacker.flag("Killaura", "D", "Combat", { attackDistance, angle });
      addHP(hurtEntity, damage);
    }
  }
}
