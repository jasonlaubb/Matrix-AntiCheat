...
system.beforeEvents.startup.subscribe((event) => {
  ...
  event.customCommandRegistry.registerEnum(
    "matrix:numberProperty",
    numberValue,
  );
  event.customCommandRegistry.registerEnum(
    "matrix:booleanProperty",
    booleanValue,
  );
  event.customCommandRegistry.registerEnum(
    "matrix:property",
      ...
      event.customCommandRegistry.registerCommand(
        ...
        (origin, ...args) => {
          ...
          if (
            ...
            (requireOp && !player.isOp())
          ) {
            return {
              status: 1,
              message:
                "Executor is not a player or command permission is invalid",
            };
          }
          for (let i = 0; i < args.length; i++) {
            const input = args[i];
            const param =
              parameters![i] ?? optionalParameters![i - parameters!.length];
            switch (param.type) {
              case "float":
              case "integer": {
              ...
              case "normalPlayerTarget": {
                ...
                if (input.length > 1)
                  return {
                    status: 1,
                    message:
                      "§7[§aMatrix§7] §fMultiple targets found. Command failed.",
                  };
                if (param.type !== "player") {
                  if (input[0].id === player.id)
                    return {
                      status: 1,
                      message:
                        "§7[§aMatrix§7] §fYou cannot target yourself with this command.",
                    };
                  if (
                    param.type === "playerTarget" &&
                    input[0].commandPermissionLevel >=
                      player.commandPermissionLevel
                  ) {
                    return {
                      status: 1,
                      message:
                        "§7[§aMatrix§7] §fYou cannot target a player with higher or equal command permission level.",
                    };
                  }
                }
                break;
