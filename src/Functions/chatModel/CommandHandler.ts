...
export function triggerCommand (player: Minecraft.Player, message: string): number {
    ...
    if (!targetCommand || (targetCommand.require && !targetCommand.require(player))) return system.run(() => {
        sendRawText (player,
            { text: "§bMatrix §7>§c " },
            { translate: "commands.generic.unknown", with: [args[0] ?? " "] },
        )
    })
    if (targetCommand.subCommand) {
        if (args.length < 2) {
            return system.run(() => sendRawText (player,
                { text: "§bMatrix §7>§c " },
                { translate: "commands.generic.syntax", with: [command, "", ""] },
            ));
        }
        ...
        if (!subCommand) {
            ...
            return system.run(() => sendRawText (player,
                { text: "§bMatrix §7>§c " },
                { translate: "commands.generic.syntax", with: [command, args[0], last] },
            ));
        }
...
export function syntaxRun (targetCommand: CommandProperties, player: Minecraft.Player, args: string[], before: string = ""): number {
    if (targetCommand.minArgs && args.length < targetCommand.minArgs) {
        return system.run(() => sendRawText (player,
            { text: "§bMatrix §7>§c " },
            { translate: "commands.generic.syntax", with: [before + args.join(" "), "", ""] },
        ));
    }
    if (targetCommand.maxArgs && args.length > targetCommand.maxArgs) {
        return system.run(() => sendRawText (player,
            { text: "§bMatrix §7>§c " },
            { translate: "commands.generic.syntax", with: [before + args.slice(0, targetCommand.maxArgs).join(" ") + " ", args.slice(targetCommand.maxArgs).join(" "), ""] },
        ));
    }
    ...
    system.run(() => {
        targetCommand.executor(player, args).catch((error) => player.sendMessage(`§bMatrix §7>§c Error:\n"` + JSON.stringify(error))).then(() => null);
    });
