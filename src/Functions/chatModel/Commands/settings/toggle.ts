...
registerCommand({
    ...
    executor: async (player, args) => {
        ...
        if (keys.includes(args[0])) {
            ...
            if ((args[1] == "enable") === getModuleState(args[0]) && args[1] != "default") return sendRawText(player, { text: "§bMatrix §7>§c " }, { translate: "toggles.already", with: [args[1]] });
            ...
            if (args[0] != "default") {
                if (world.getDynamicProperty(args[0]) == (args[1] == "enable")) return sendRawText(player, { text: "§bMatrix §7>§c " }, { translate: "toggles.already", with: [args[1]] });
