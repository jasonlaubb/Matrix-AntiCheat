    ...
    },
    {
        ...
        executor: async (player, args) => {
            ...
            if (!player.hasTag(`rank:${args[1]}`)) {
                ...
                sendRawText(player, { text: "§bMatrix §7>§c " }, { translate: "rank.already", with: [target.name, args[1]] });
            }
        },
    },
    {
        ...
        executor: async (player, args) => {
            ...
            if (target.getTags().filter((rank) => rank.startsWith("rank:")).length > 0) {
                ...
                sendRawText(player, { text: "§bMatrix §7>§c " }, { translate: "rank.empty", with: [target.name] });
            }
        },
    }
);
