...
world.beforeEvents.entityRemove.subscribe(({ removedEntity: entity }) => {
    ...
    //flag the player
    system.run(() =>
       flag(player, "CommandBlockExplolit", config.antiCommandBlockExplolit.maxVL, config.antiCommandBlockExplolit.punishment, [lang(">distance") + ":" + distance.toFixed(2), lang(">Pos") + ":" + Math.floor(pos.x) + ", " + Math.floor(pos.y) + ", " + Math.floor(pos.z)])
    )
})