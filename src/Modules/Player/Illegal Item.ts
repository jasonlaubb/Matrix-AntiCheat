import { world, system, Container, Player, ItemStack, ItemEnchantableComponent, EntityInventoryComponent, BlockInventoryComponent, PlayerPlaceBlockBeforeEvent } from "@minecraft/server";
...
function ItemCheck(player: Player, container: Container): "Safe" | "Unsafe" {
    ...
    for (let i = 0; i < container.size; i++) {
        ...
        if (config.antiIllegalItem.state.typeCheck.enabled && config.antiIllegalItem.illegalItem.includes(item.typeId)) {
            container.setItem(i);
            flag(player, "Illegal Item", "A", 0, config.antiIllegalItem.state.typeCheck.punishment, ["Mode" + ":" + "ItemType", "typeId" + ":" + item.typeId]);
            state = "Unsafe";
            continue;
        }
        ...
        if (config.antiIllegalItem.state.nameLength.enabled && item.nameTag && item.nameTag?.length > config.antiIllegalItem.state.nameLength.maxItemNameLength) {
            container.setItem(i);
            flag(player, "Illegal Item", "B", 0, config.antiIllegalItem.state.nameLength.punishment, ["Mode" + ":" + "ItemNameLength", "nameLength" + ":" + item.nameTag?.length]);
            state = "Unsafe";
            continue;
        }
        ...
        if (config.antiIllegalItem.state.itemTag.enabled && (item.getCanPlaceOn().length > config.antiIllegalItem.state.itemTag.maxAllowedTag || item.getCanDestroy().length > config.antiIllegalItem.state.itemTag.maxAllowedTag)) {
            container.setItem(i);
            flag(player, "Illegal Item", "C", 0, config.antiIllegalItem.state.itemTag.punishment, ["Mode" + ":" + "ItemTag"]);
            state = "Unsafe";
            continue;
        }
        ...
        if (config.antiIllegalItem.state.loreCheck.enabled) {
            ...
            if (lore.length > 0) {
                ...
                flag(player, "Illegal Item", "D", 0, config.antiIllegalItem.state.loreCheck.punishment, ["Mode" + ":" + "ItemLore", "ItemLore" + ":" + truncateString(loreString)]);
                state = "Unsafe";
                continue;
            }
        }
        ...
        if (config.antiIllegalItem.state.itemAmount.enabled && (item.amount > item.maxAmount || item.amount <= 0)) {
            container.setItem(i);
            flag(player, "Illegal Item", "E", 0, config.antiIllegalItem.state.itemAmount.punishment, ["Mode" + ":" + "ItemAmount", "Amount" + ":" + item.amount]);
            state = "Unsafe";
            continue;
        }
        ...
        if (config.antiIllegalItem.state.enchantLevel.enabled || config.antiIllegalItem.state.enchantConflict.enabled) {
            ...
            if (patchedEnchantment.length > 0) {
                flag(player, "Illegal Item", type, 0, config.antiIllegalItem.state.enchantLevel.punishment, ["Mode" + ":" + mode, ...patchedEnchantment]);
                state = "Unsafe";
                container.setItem(i);
                continue;
            }
        ...
        if (config.antiIllegalItem.state.enchantAble.enabled && enchantments.length > 0 && !enchantableItem.includes(item.typeId as MinecraftItemTypes) && !config.antiIllegalItem.state.enchantAble.whiteList.includes(item.typeId)) {
            container.setItem(i);
            flag(player, "Illegal Item", "H", 0, config.antiIllegalItem.state.enchantAble.punishment, ["Mode" + ":" + "ItemEnchantAble", "typeId" + item.typeId]);
            state = "Unsafe";
            continue;
        }
        ...
        if (config.antiIllegalItem.state.enchantRepeat.enabled && new Set(enchantments).size < enchantments.length) {
            container.setItem(i);
            flag(player, "Illegal Item", "I", 0, config.antiIllegalItem.state.enchantRepeat.punishment, ["Mode" + ":" + "ItemEnchantRepeat"]);
            state = "Unsafe";
            continue;
        }
    }
