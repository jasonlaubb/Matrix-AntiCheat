...
async function antiAuto (player: Player) {
    if (player.hasTag("matrix:moving") && player.isOnGround && !player.isJumping && !player.isGliding && !player.hasTag("matrix:riding")) {
        flag (player, "Auto Totem", config.antiAuto.maxVL, config.antiAuto.punishment, [lang(">Type") + ":" + lang(">Moving")])
    } else

    if (player.hasTag("matrix:usingItem")) {
        flag (player, "Auto Totem", config.antiAuto.maxVL, config.antiAuto.punishment, [lang(">Type") + ":" + lang(">UsingItem")])
    } else

    if (player.hasTag("matrix:container")) {
        flag (player, "Auto Totem", config.antiAuto.maxVL, config.antiAuto.punishment, [lang(">Type") + ":" + lang(">Container")])
    }
}
