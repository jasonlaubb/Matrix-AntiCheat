...
system.runInterval(()=>{
  for(let player of world.getPlayers()){
    ...
    UI.body("You have to verify you are not bot by press verify")
    UI.button("§a§lVerify")
    UI.show(player).then((result)=>{
      ...
      } if(result.selection == 0 && Date.now() - clickSpeed.get(player.id) > config.antiARAS.clickSpeedThershold*50){
        if(!player.hasTag("matrix:notVerified")) return
      ...
      } if(Date.now() - clickSpeed.get(player.id) <= config.antiARAS.clickSpeedThershold*50 && result.selection  == 0){
