...
async function antiSpammer (player: Player) {
    ...
    if (player.hasTag('matrix:attack_time')) {
        flag (player, "Spammer", config.antiSpammer.maxVL, config.antiSpammer.punishment, [lang(">Type")+":"+lang(">AttackTime")])
        player.applyDamage(6)
    } else

    //check if the player send message while using item
    if (player.hasTag('matrix:using_item')) {
        flag (player, "Spammer", config.antiSpammer.maxVL, config.antiSpammer.punishment, [lang(">Type")+":"+lang(">UsingItem")])
        player.applyDamage(6)
    } else
    ...
    if (player.hasTag("matrix:moving") && player.hasTag("matrix:riding") && player.isOnGround && !player.isJumping && !player.isInWater && !player.isGliding) {
        flag (player, "Spammer", config.antiSpammer.maxVL, config.antiSpammer.punishment, [lang(">Type")+":"+lang(">Moving")])
        player.applyDamage(6)
    }
    
    //check if the player sned message opening container
    if (player.hasTag("matrix:container")) {
        flag (player, "Spammer", config.antiSpammer.maxVL, config.antiSpammer.punishment, [lang(">Type")+":"+lang(">Container")])
        player.applyDamage(6)
    }
}
