...
async function AntiFly (player: Player, now: number) {
    ...
    const clientFly = velocityLog[player.id] > 1 && player?.lastVelLog == velocityLog[player.id]

    const skip1 = !(player.lastExplosionTime && now - player.lastExplosionTime < 5500) && !(player.threwTridentAt && now - player.threwTridentAt < 5000)
...
if(velocity> 0.7 && xz > 1 && ! player.isOnGround && !instair){
	player.teleport(prevLoc) 
...
//efficiency: high | false postive: unknown 
    ...
    if (player.isOnGround && velocity > 0) player.addTag("matrix:runned_velocity")
