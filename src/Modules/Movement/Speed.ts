...
async function AntiSpeedA2(player: any, now: number) {
    ...
    if (isAdmin(player) || player.isGliding || player.hasTag("matrix:riding")) return;
    //define some cool things
    const config = c();
    //const { x, z } = player.getVelocity();
    //const xz = Math.hypot(x, z);
    //dBVD == difference between velocity and moved distance
    const dBVD = Math.abs(xzLog[player.id] - disLog[player.id]) || yDisLog[player.id] - yLog[player.id];
    //setting max value of dBVD
    //iSL = is spike lagging
    if (iSL[player.id]) maxDBVD[player.id] = 1;
    ...
    if (dBVD > maxDBVD[player.id] || timerLog[player.id] >= 3) {
        ...
        if (!iSL[player.id] && ((dBLFN < 5000 && timerLog[player.id] >= 3) || (dBLFN < 1100 && dBVD > maxDBVD[player.id])))
