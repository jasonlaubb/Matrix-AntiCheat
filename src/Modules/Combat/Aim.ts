...
x   const config = c();
    ...
    const unnaturalRots = (rotationx == rotationx.toFixed(4) && (rotationX != 0 || rotSpeedY > 0 && lastRotationX == 0 && rotationX == 0)) || rotationY == rotation.toFixed(4) && (rotationY != 0) || rotSpeedX == rotSpeedX.toFixed(4) || rotSpeedY == rotSpeedY.toFixed(4)) 
    if (unnaturalRots){
       flag(player, "Aim", "G", config.antiAim.maxVL, config.antiAim.punishment, undefined);
    } 
    const instantRot = (lastRotSpeedX >= 30 && rotSpeedX <= 2 && (rotationX != 0 || rotSpeedY > 0 && lastRotationX == 0 && rotationX == 0) || lastRotSpeedY >= 30 && rotSpeedY <= 2 && (rotationY != 0 || rotSpeedX > 0 && lastRotationY == 0 && rotationY == 0)) 
    if (instantRot){
