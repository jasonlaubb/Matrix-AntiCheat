...
async function onPlayerSpawn(config: configi, event: PlayerSpawnAfterEvent) {
    ...
    while (now - Date.now() < config.realmDefender.maxAllowanceTime && checkValid) {
        ...
        const ui = new ModalFormData()
            ...
            .slider(new rawstr().tra("defender.selectui").str("\n").tra("defender.why").str("\n").tra("defender.question", question1.toString(), question2.toString()).str("\n").tra("defender.youranswer").parse(), 0, 10, 1, ran(11))
