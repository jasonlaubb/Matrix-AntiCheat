...
registerCommand({
    ...
    executor: async (player, args) => {
        ...
        const config = c();
        const correctPassword = config.commands.passwordSetting.usingHash ? SHA256(config.commands.passwordSetting.password).toString() : config.commands.passwordSetting.password;
